...
const bool writeBMP24(const char *filename, const int x_res, const int y_res, const RGB* rgb_array)
{
	...
	for (i = 0; i < height; i++)
	{
		...
		{
			ipos = 3 * (width * i + j);
			buffer[3 * j]	  = rgb_array[(j + i * x_res)].red_; // blue
			buffer[3 * j + 1] = rgb_array[(j + i * x_res)].green_; // green
			buffer[3 * j + 2] = rgb_array[(j + i * x_res)].blue_;	// red
		}
    ...
    *rgb_array = (RGB*)malloc(sizeof(RGB)*(width * height));
    ...
    for (int y = 0; y < height; y++)
        for (int x = 0; x < 3 * width; x += 3)
        {
            ...
            (*rgb_array)[new_pos].red_ = (float)image_buf[buf_pos + 0];
            (*rgb_array)[new_pos].green_ = (float)image_buf[buf_pos + 1];
            (*rgb_array)[new_pos].blue_ = (float)image_buf[buf_pos + 2];
        }
